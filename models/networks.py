...
# modification of the standard resnet generator
class ResnetGenerator_embedding(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        """
        assert (n_blocks >= 0)
        super(ResnetGenerator_embedding, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        self.embedding = nn.Sequential(*model)
    ...
    def forward(self, input):
        """Standard forward"""
        return self.model(input), self.embedding(input)
...
if __name__ == "__main__":
    ...
    import numpy as np
    thres = np.load("/Users/francescodifeola/Desktop/thresholdssubset7.npy")
    print(thres)
    d = ResnetGenerator_embedding(input_nc=1, output_nc=1)
    print(d)
    t = torch.ones(16, 1, 256, 256)
    print(t.shape)
    a, b = d(t)
    combined_embedding = torch.stack([b, b, b, b], dim=1)
    print(combined_embedding.shape)
    print(a.shape, b.shape)
    condensed_embedding = nn.Conv3d(4, 1, kernel_size=1)(combined_embedding)
    print(condensed_embedding.shape)
