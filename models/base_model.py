...
class BaseModel(ABC):
    ...
    def get_current_weights(self):
        ...
        for name in self.weights_names:
            if isinstance(name, str):
                errors_ret[name] = float(
                    getattr(self, 'weight_' + name))  # float(...) works for both scalar tensor and float number
        return errors_ret
    ...
    def track_loss_weights(self):
        """Return weights for traning losses."""
        for name in self.weights_names:
            if isinstance(name, str):
                self.weights_store[name].append(
                    float(getattr(self, 'weight_' + name)))  # float(...) works for both scalar tensor and float number
        return self.weights_store
