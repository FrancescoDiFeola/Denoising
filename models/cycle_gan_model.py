...
from loss_functions.LSTM import AdaptiveLSTM, CoeffOfVariation
...
def z_score_normalize(vector):
    ...
    if std_dev == 0:
        ...
        normalized_vector = torch.zeros_like(vector)
    else:
        normalized_vector = (vector - mean_val) / std_dev

    return normalized_vector


def denormalize_zscore(normalized_vector, original_mean, original_std):
    denormalized_vector = normalized_vector * original_std + original_mean
...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.experiment_name.find('texture') != -1:
            ...
            self.weights_names = ['a11', 'a12', 'a13', 'a14', 'a21', 'a22', 'a23', 'a24', 'a31', 'a32', 'a33', 'a34', 'a41', 'a42', 'a43', 'a44']
        ...
        # dictionary to store training loss weights
        self.weights_store = OrderedDict()
        for key in self.weights_names:
            self.weights_store[key] = list()
        ...
        self.adaptive_lstm = AdaptiveLSTM(input_size=4, hidden_size=10, output_size=4)
        ...
        if self.isTrain:  # define discriminators
            ...
            if opt.texture_criterion == 'attention':
                ...
                self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters(), self.adaptive_lstm.parameters()),
                                                    lr=opt.lr, betas=(opt.beta1, 0.999))
            ...
            self.CoV = CoeffOfVariation(self.opt, self.device)
    ...
    def backward_G(self):
        ...
        # weights = self.loss_weights.clone().detach()
        # self.weight_lambda_A = 10  # weights[0][0][0][0]
        # self.weight_lambda_B = 10  # weights[0][0][0][1]
        # self.weight_identity_A = 5  # weights[0][0][0][2]
        # self.weight_identity_B = 5  # weights[0][0][0][3]

        # Identity loss
        if lambda_idt > 0:
            # G_A should be identity if real_B is fed: ||G_A(B) - B||
            self.idt_A = self.netG_A(self.real_B)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * 20  # * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_A)
            self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * 20  # * lambda_A * lambda_idt
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
        ...
        print(self.criterionCycle(self.rec_A, self.real_A) * 10)
        self.loss_cycle_A = self.criterionCycle(self.rec_A, self.real_A) * 40  # * lambda_A
        print(self.criterionCycle(self.rec_A, self.real_A) * 40)
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * 40  # * lambda_B

        ####################################################
        # Attention weighting
        """self.loss_weights = torch.stack([self.loss_cycle_A, self.loss_cycle_B, self.loss_idt_A, self.loss_idt_B], dim=-1).unsqueeze(0).unsqueeze(1).unsqueeze(2)
        print(self.loss_weights)
        norm = torch.norm(self.loss_weights, p=2, dim=-1)
        normalized_loss_weights = (self.loss_weights - self.loss_weights.min())/(self.loss_weights.max() - self.loss_weights.min())  # F.normalize(self.loss_weights, p=2, dim=-1)
        print(normalized_loss_weights)
        # print(normalized_loss_terms)
        attention_loss_weights, weights, gamma = self.netattention(normalized_loss_weights)
        self.weight_a11 = weights[0][0][0]
        self.weight_a12 = weights[0][0][1]
        self.weight_a13 = weights[0][0][2]
        self.weight_a14 = weights[0][0][3]
        self.weight_a21 = weights[0][1][0]
        self.weight_a22 = weights[0][1][1]
        self.weight_a23 = weights[0][1][2]
        self.weight_a24 = weights[0][1][3]
        self.weight_a31 = weights[0][2][0]
        self.weight_a32 = weights[0][2][1]
        self.weight_a33 = weights[0][2][2]
        self.weight_a34 = weights[0][2][3]
        self.weight_a41 = weights[0][3][0]
        self.weight_a42 = weights[0][3][1]
        self.weight_a43 = weights[0][3][2]
        self.weight_a44 = weights[0][3][3]
        print(gamma)
        print(weights)
        self.loss_weights = attention_loss_weights * norm.view(-1, 1)
        print(self.loss_weights)"""

        loss_terms = torch.stack([self.loss_cycle_A, self.loss_cycle_B, self.loss_idt_A, self.loss_idt_B], dim=-1)

        CoVs, current_iter = self.CoV(loss_terms)
        print(CoVs)
        if current_iter <= 2:

            self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B
        else:
            self.weights = self.adaptive_lstm(CoVs.unsqueeze(0))
            print(self.weights)
            """self.loss_cycle_A = self.weights[0][0] * loss_terms[0]
            self.loss_cycle_B = self.weights[0][1] * loss_terms[1]
            self.loss_idt_A = self.weights[0][2] * loss_terms[2]
            self.loss_idt_B = self.weights[0][3] * loss_terms[3]"""

            print(self.weights[0][0] * loss_terms[0], self.weights[0][1] * loss_terms[1], self.weights[0][2] * loss_terms[2], self.weights[0][3] * loss_terms[3])
            self.loss_G = self.loss_G_A + self.loss_G_B + self.weights[0][0] * loss_terms[0] + self.weights[0][1] * loss_terms[1] + self.weights[0][2] * loss_terms[2] + self.weights[0][3] * loss_terms[3]  # inizializzo i pesi a 20 e 10 cosi' all'inzio sto nell'intorno dei valori di default"""
