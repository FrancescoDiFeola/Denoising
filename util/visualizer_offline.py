...
class Visualizer():
    ...
    def plot_current_weights(self, epoch, tracked_weights, name_title):
        loss_path = os.path.join(self.loss_dir, f'weights_epoch.png')
        if epoch == 50 or epoch == 200:
            util.save_ordered_dict_as_csv(tracked_weights, f'{self.loss_dir}/weights_loss.csv')
        util.save_plots(tracked_weights, loss_path, name_title)  # save the plots to the disk
